<?php

/**
 * PasswordGenerator class file
 * 
 * @author Gayan Chandima
 * @copyright Copyright&copy; 2011 SoNET Systems
 */

/**
 * This class is used to format/ generate passwords
 *
 * @package am.admin.components
 */
class PasswordGenerator {

   /**
    *
    * @varString
    */
   private $password;

   /**
    * returns password
    * @return String
    */
   public function getPassword() {
      return $this->password;
   }

   /**
    * generates a random password
    * 
    * @param Int $PwdLength
    * @param String $PwdType
    * @return String returns encripted password
    */
   public function randomPassword($PwdLength=8, $PwdType='standard') {
      $Ranges = '';

      if ('test' == $PwdType)
         return 'test';
      elseif ('standard' == $PwdType)
         $Ranges = '65-78,80-90,97-107,109-122,50-57';
      elseif ('alphanum' == $PwdType)
         $Ranges = '65-90,97-122,48-57';
      elseif ('any' == $PwdType)
         $Ranges = '40-59,61-91,93-126';

      if ($Ranges <> '') {
         $Range = explode(',', $Ranges);
         $NumRanges = count($Range);
         mt_srand(time()); //not required after PHP v4.2.0
         $p = '';
         for ($i = 1; $i <= $PwdLength; $i++) {
            $r = mt_rand(0, $NumRanges - 1);
            list($min, $max) = explode('-', $Range[$r]);
            $p.=chr(mt_rand($min, $max));
         }
         return $p;
      }
   }
}
