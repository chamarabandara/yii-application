...
class SiteController extends Controller {
    ...
    public function actionGetAdvertisementList() {
        ...
        try {
            ...
            $longitudeval = $this->checkNum($_POST['longitude']);
            $latitudeval = $this->checkNum($_POST['latitude']);
            $last_update_date = $_POST['lastupdate']; //         
            //initialize basic response object set
            $advertisementD = new OutPutObj();
            $advertisementD->response = new ResponseObj();
            ...
            foreach ($categories as $cat) {
                ...
                foreach ($subCategoryList as $value) {
                    ...
                    if ($subcatins->sub_category_id == '-1') {
                        $advertisementList = CategoryHasSubCategory::model()->with('advertisement')->findall(
                                array(
                                    'select' => '*',
                                    'condition' => 't.category_id=:cat_id AND advertisement.status="Active" AND advertisement.updated_date>:updated_date AND advertisement.exp_date > :exp_date AND advertisement.is_featured=:fetured',
                                    'params' => array(':cat_id' => $cat->id, ':exp_date' => date('Y-m-d'), ':fetured' => Advertisement::CATEGORY_FEATURED_ADD, ':updated_date' => $last_update_date),
                                )
                        );
                        $arraySub = array();
                        ...
                        $subFeaturedList = CategoryHasSubCategory::model()->with('advertisement')->findAll(array(
                            'select' => '*',
                            'condition' => 't.sub_category_id=:scat_id AND t.category_id=:cat_id AND advertisement.status="Active" AND advertisement.exp_date > :exp_date AND advertisement.is_featured=:fetured AND advertisement.updated_date>:updated_date',
                            'params' => array(':cat_id' => $cat->id, ':exp_date' => date('Y-m-d'), ':scat_id' => $value->subCategory->id, ':fetured' => Advertisement::SUB_CATEGORY_FEATURED_ADD, ':updated_date' => $last_update_date),
                        ));
                        $arraySub = array();
                        $cat_i = 0;
                        ...
                        $advertisementList = CategoryHasSubCategory::model()->with('advertisement')->findall(
                                array(
                                    'select' => '*',
                                    'condition' => 't.category_id=:cat_id AND t.sub_category_id=:sub_id AND advertisement.status="Active" AND advertisement.exp_date > :exp_date AND advertisement.updated_date>:updated_date',
                                    'params' => array(':cat_id' => $cat->id, ':exp_date' => date('Y-m-d'), ':sub_id' => $value->subCategory->id, ':updated_date' => $last_update_date),
                                )
                        );
                    }
                    ...
                    foreach ($advertisementList as $addValue) {
                        ...
                        $objadvertisement->promo_text = $addValue->advertisement->promo_text;
                ...
                $catogoryList = Advertisement::model()->with('categoryHasSubCategories')->findAll(array(
                    'select' => '*',
                    'condition' => 'updated_date>=:updated_date AND t.category_id=:cat_id AND t.status="Active" AND t.exp_date > :exp_date AND t.is_featured=:fetured',
                    'params' => array(':cat_id' => $cat->id, ':exp_date' => date('Y-m-d'), ':fetured' => Advertisement::CATEGORY_FEATURED_ADD, ':updated_date' => $last_update_date)
                ));
                $array = array();
                $cat_i = 0;
    ...
    public function actionGetAdvertisementDetail() {
        ...
        if ($advertisement != null) {
            ...
            $AdvertisementDetailDataObj->promo_text = $advertisement->promo_text;
    ...
    public function actionGetAdvertisementAllDetail() {
        ...
        foreach ($addvertisment as $addvertisment) {
            ...
            $arr['promo_text'] = $addvertisment->advertisement->promo_text;
