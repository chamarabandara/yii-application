            ...
            foreach ($categories as $cat) {
                ...
                foreach ($subCategoryList as $value) {
                    ...
                    if ($subcatins->sub_category_id == '-1') {
                        $advertisementList = CategoryHasSubCategory::model()->with('advertisement')->findall(
                                array(
                                    ...
                                    'params' => array(':cat_id' => $cat->id, ':exp_date' => date('Y-m-d'), ':fetured' => Advertisement::CATEGORY_FEATURED_ADD, ':updated_date' => substr($last_update_date, 0, 11)),
                                )
                        );
                    } else {
                        ...
                        $subFeaturedList = CategoryHasSubCategory::model()->with('advertisement')->findAll(array(
                            ...
                            'params' => array(':cat_id' => $cat->id, ':exp_date' => date('Y-m-d'), ':scat_id' => $value->subCategory->id, ':fetured' => Advertisement::SUB_CATEGORY_FEATURED_ADD,':updated_date' => substr($last_update_date, 0, 11)),
                        ));
                        $arraySub = array();
                        $cat_i = 0;
                        ...
                        $advertisementList = CategoryHasSubCategory::model()->with('advertisement')->findall(
                                array(
                                    ...
                                    'params' => array(':cat_id' => $cat->id, ':exp_date' => date('Y-m-d'), ':sub_id' => $value->subCategory->id,':updated_date' => substr($last_update_date, 0, 10)),
                                )
                        );
                    }
                    ...
                    foreach ($advertisementList as $addValue) {
                        ...
                        $objadvertisement->offer_description = $addValue->advertisement->tag_line;
                ...
                $catogoryList = Advertisement::model()->with('categoryHasSubCategories')->findAll(array(
                    'select' => '*',
                    'condition' => 't.category_id=:cat_id AND t.status="Active" AND t.exp_date > :exp_date AND t.is_featured=:fetured AND t.updated_date>=:updated_date',
                    'params' => array(':cat_id' => $cat->id, ':exp_date' => date('Y-m-d'), ':fetured' => Advertisement::CATEGORY_FEATURED_ADD, ':updated_date' => substr($last_update_date, 0, 11)),
                ));
                $array = array();
                $cat_i = 0;
